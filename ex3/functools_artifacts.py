...
def spell_reducer(spells: list[int], operation: str) -> int:
    if (not isinstance(spells, list) or
        ...
        raise ValueError("spells must be a list of int")
    if not spells:
        return 0
...
def base_enchantment(power: int, element: str, target: str) -> str:
    if (not isinstance(power, int) or
        ...
        raise ValueError("power must be an int, element must "
...
def partial_enchanter(base_enchantment: Callable) -> dict[str, Callable]:
    if not callable(base_enchantment):
        raise TypeError("base_enchantment must be a callable")
...
if __name__ == '__main__':
    ...
    try:
        ...
        for lst in lst_fibo:
            ...
            if lst < 0:
                raise ValueError(' n must be an int and >= 0')
