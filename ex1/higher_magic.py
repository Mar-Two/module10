...
def heal(target: str, power: int) -> str:
    if not isinstance(target, str) or not isinstance(power, int):
        raise ValueError("Error: target must be a str "
                         "and power must be an int")
    return f"Heal restores {target} for {power} HP"


def fireball(target: str, power: int) -> str:
    if not isinstance(target, str) or not isinstance(power, int):
        raise ValueError("Error: target must be a str and "
                         "power must be an int")
...
def spell_combiner(spell1: Callable, spell2: Callable) -> Callable:
    if not callable(spell1) or not callable(spell2):
        raise TypeError("Error: spell1 and spell2 must be callable")
    return lambda target, power: (spell1(target, power), spell2(target, power))


def power_amplifier(base_spell: Callable, multiplier: int) -> Callable:
    if not isinstance(multiplier, int):
        raise ValueError("Error: multiplier must be an int")
    if not callable(base_spell):
        raise TypeError("Error: base_spell must be callable")
    return lambda target, power: base_spell(target, power * multiplier)


def conditional_caster(condition: Callable, spell: Callable) -> Callable:
    if not callable(condition) or not callable(spell):
        raise TypeError("Error: condition and spell must be a callable")
...
def spell_sequence(spells: list[Callable]) -> Callable:
    if not all(callable(x) for x in spells):
        raise TypeError("Error: All items in spells list must be callable")
