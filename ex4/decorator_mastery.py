...
def spell_timer(func: Callable) -> Callable:
    if not callable(func):
        raise TypeError("Error: func must be callable")
...
def power_validator(min_power: int):
    if not isinstance(min_power, int):
        raise TypeError("Error: min_power must be an int")
...
def retry_spell(max_attempts: int) -> Callable:
    if not isinstance(max_attempts, int) or max_attempts < 1:
        raise ValueError("Error: max_attempts must be an int >= 1")
...
class MageGuild:
    ...
    def validate_mage_name(name: str) -> bool:
        if not isinstance(name, str):
            raise ValueError("Error: name must be a str")
